...
export function MapView({
  ...
  useEffect(() => {
    ...
    const render = (now: number): void => {
      ...
      // Country badges + sprites. Text crispness comes from the canvas no
      // longer using `image-rendering: pixelated`, so we draw at native
      // dimensions and let CSS scale handle zoom.
...
function drawCountryBadges(
  ...
  for (const c of map.countries) {
    ...
    // Native-sized badge. Text crispness now comes from a non-pixelated
    // canvas; fonts use sub-pixel anti-aliasing at any zoom.
